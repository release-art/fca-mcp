...
class FcaAiAssistant:
    ...
    async def analyze_firm_risk(self, firm_name: str) -> dict[str, Any]:
        ...
        search_result = await self.server.handle_request(
            tool="search_firms",
            params={"query": firm_name, "limit": 1},
            authorization=None
        )
        ...
        history_result = await self.server.handle_request(
            tool="firm_related",
            params={"firm_id": firm_id, "kind": "history", "mode": "full"},
            authorization=None
        )
        ...
        requirements_result = await self.server.handle_request(
            tool="firm_related",
            params={"firm_id": firm_id, "kind": "requirements", "mode": "full"},
            authorization=None
        )
    ...
    def _generate_recommendation(self, risk_score: dict, history: list) -> str:
        ...
        if level == "HIGH RISK":
            return ("HIGH RISK: This firm presents significant regulatory concerns. "
                    "Recommend thorough due diligence before any business engagement.")
        elif level == "MEDIUM RISK":
            return ("MEDIUM RISK: This firm has some regulatory concerns. "
                    "Review disciplinary history and requirements carefully.")
        else:
            return ("LOW RISK: This firm has a good regulatory standing. "
    ...
    async def compare_firms(self, firm1_name: str, firm2_name: str) -> dict[str, Any]:
        ...
        comparison = {
            ...
            "recommendation": (f"Based on regulatory data, {lower_risk} presents a lower risk profile. "
    ...
    async def get_firm_permissions_summary(self, firm_name: str) -> dict[str, Any]:
        ...
        perms_result = await self.server.handle_request(
            tool="firm_related",
            params={"firm_id": firm_id, "kind": "permissions", "mode": "full"},
            authorization=None
        )
...
class FcaMcpServerRunner:
    ...
    async def start(self, enable_auth: bool = False) -> None:
        ...
        print(f"[INIT] Connecting to FCA API...")
    ...
    async def handle_related(self, frn: str, kind: str) -> None:
        ...
        result = await self.server.handle_request(
            tool="firm_related",
            params={"firm_id": frn, "kind": kind, "mode": "full"},
            authorization=None
        )
...
if FASTAPI_AVAILABLE:
    ...
    async def lifespan(app: FastAPI):
        ...
        _global_server = await create_server(
            fca_email=fca_email,
            fca_key=fca_key,
            enable_auth=False
        )
...
def main_http_mode() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="FCA MCP Server - HTTP Mode",
...
HTTP endpoints:
  ...
  POST /api/ask                                # Natural language query
    ...
    parser.add_argument(
        "--no-interactive",
        action="store_true",
        help="Run in non-interactive mode (server only)"
    )
    parser.add_argument(
        "--enable-auth",
        action="store_true",
        help="Enable OAuth2 authentication"
    )
